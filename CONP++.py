...
PATH_PRE = 'E:\Experiments Codes\LF-GCN\pretrain_weights\ijba_res18_naive.pth.tar'
path_list = 'F:\DataSets\RAFDB\\'
path_image = 'F:\DataSets\RAFDB\\aligned\\'
...
class FERResNet18(nn.Module):
    ...
    def __init__(self, num_classes=7,attention=False):
        ...
        # self.features 提取前面所有层    self.features2 仅提取倒数第二个层
    ...
    def forward(self, x):
        ...
        output_N = self.fc_N(feature_n)  # 使用新的 fc 层
...
def find_min_items_to_reach_threshold(batch, th):
    """
    对于每个样本的向量，找到最少的几个元素使得它们的和大于th。

    Args:
    - batch: 一个形状为 (batch_size, c) 的张量，表示批量数据
    - th: 阈值 (float)

    Returns:
    - min_items: 一个列表，表示每个样本满足条件的最少项数
    """
    # 对每个样本进行降序排序
    sorted_batch, sorted_indices = torch.sort(batch, descending=True, dim=1)  # (batch_size, c)

    # 计算前缀和
    prefix_sum = torch.cumsum(sorted_batch, dim=1)  # 前缀和，形状为 (batch_size, c)

    # 创建一个与前缀和相同形状的mask，判断每个元素是否大于阈值
    mask = prefix_sum > th  # (batch_size, c)

    # 找到第一个大于阈值的索引
    min_items = mask.float().argmax(dim=1) + 1  # +1 是因为索引是从0开始的，表示实际项数

    indices_mask = torch.zeros_like(batch, dtype=torch.bool)  # 初始化掩码
    for i in range(batch.size(0)):  # 遍历每个样本
        # 根据原始索引位置生成掩码
...
def quick_masked_batch_topk(input_tensor,change_tensor, k_tensor):
    """
    对 batch_size 中的每个样本按顺序保留负学习中 top-k 最大值，表明不是这几类，并且按照这几类的位置，将正学习中的对应位置填充为 0，保留输入正学习顺序。

    Args:
    - input_tensor (Tensor): 输入负学习意见张量，形状为 (batch_size, c)。
    - change_tensor (Tensor): 输入正学习意见张量，形状为 (batch_size, c)。
    - k_tensor (Tensor): 每个样本的 k 值，形状为 (batch_size,)。

    Returns:
    - masked_tensor (Tensor): 按顺序保留 top-k 最大值的张量，形状为 (batch_size, c)。
    - mask_indices (Tensor): 被保留值的索引，形状为 (batch_size, c)，非保留部分索引为 -1。
    """
    batch_size, c = input_tensor.shape

    # 对输入张量逐行计算 top-k 阈值
    sorted_tensor, _ = torch.sort(input_tensor, dim=1, descending=True)
    topk_threshold = sorted_tensor[torch.arange(batch_size), k_tensor - 1]  # 每行的第 k 个最大值作为阈值

    # 生成掩码，保留 < 阈值的元素 说明这些元素对应的位置可能是这一类 用这个信息优化正学习
    mask = input_tensor < topk_threshold.unsqueeze(1)
    # 按掩码保留原张量中的值，非保留部分置为 0
...
def trian(model, Pnet, set_loader, eps, lr, sch, globalbest,th):
    ...
    for epoch in range(EPOCHS):
        ...
        for item in tqdm(set_loader):
            ...
            # CONP base： 正学习获得概率分布 将大于平均概率的置为1 视为K个候选类 根据此生成多标签负标签 监督负网络 输出意见筛选出Classnum-K个不可信类  掩盖正学习对应位置概率 重新归一化 使用这个概率对正学习进行蒸馏
            # CONP++: 1: 候选类自适应 2:负学习置信度加权
            ...
            # 自适应K
            probs = F.softmax(outputs_P, dim=1)  # 获取每个样本的概率分布
            # NO ACN
